...
(
  ...
  if [[ "${_CONFIG}" =~ (zero|bldtst|pico) ]]; then
    ...
    options+=' -DCURL_DISABLE_DICT=ON -DCURL_DISABLE_FILE=ON -DCURL_DISABLE_GOPHER=ON -DCURL_DISABLE_MQTT=ON -DCURL_DISABLE_RTSP=ON -DCURL_DISABLE_TELNET=ON -DCURL_DISABLE_TFTP=ON'
    ...
    options+=' -DCURL_DISABLE_HTTPSIG=OFF'
  fi
  ...
  if [[ "${_CONFIG}" = *'nohttp'* || "${_CONFIG}" = *'noproxy'* ]]; then
  ...
  if [[ "${h3}" = '1' && \
    ...
    if [ -n "${_OPENSSL}" ] && [ -d "../${_OPENSSL}/${_PP}" ]; then
      options+=' -DUSE_PROXY_HTTP3=ON'  # experimental
    fi
  else
